...
async function generateName() {
    ...
    if(wikiCheck) {
        ...
        if(maybeItExists) {
            checkE.innerHTML = `${pre}${suf} <a href="https://en.wikipedia.org/w/index.php?search=MetroTrans${pre}${suf}" target="_blank" rel="noopener noreferrer">might actually exist</a>!`;
        } else {
            checkE.innerHTML = `It doesn\'t look like ${pre}${suf} <a href="https://en.wikipedia.org/w/index.php?search=MetroTrans${pre}${suf}" target="_blank" rel="noopener noreferrer">exists</a> :(`;
        }
    }
}
